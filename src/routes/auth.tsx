...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Access — SoundXpand" },
      {
        name: "description",
        content:
          "Sign in to SoundXpand or create a free distribution account for artists, labels, songwriters and publishers.",
      },
...
function AuthTabs() {
  ...
  useEffect(() => {
    getPublicPlatformFlagsFn()
      .then((f) => setAutoApprove(!!f.auto_approve))
      .catch(() => {});
  }, []);
  useEffect(() => {
    const sync = () => {
...
function RegisterForm() {
  ...
  const [form, setForm] = useState<Partial<RegisterForm>>({
    ...
    main_genre: "",
    current_distributor: "",
    tracks_released_bucket: "",
    spotify_monthly_listeners_bucket: "",
    private_link: "",
    social_instagram: "",
    social_facebook: "",
    social_tiktok: "",
    ...
    return (
      <div className="text-center py-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          We sent a verification link to <span className="text-foreground">{form.email}</span>. After verifying, your
          account will be reviewed by an administrator.
        </p>
      </div>
    );
  }
  ...
  if (step === 1) {
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <Label>Your name (Artist, Band, Label) *</Label>
          <Input required value={form.artist_name} onChange={(e) => set("artist_name")(e.target.value)} />
        </div>
        ...
        <div className="space-y-1">
          <Label>Private link for your next release/project</Label>
          <Input
            value={form.private_link}
            onChange={(e) => set("private_link")(e.target.value)}
        ...
        <label className="flex items-start gap-2 text-xs text-muted-foreground pt-2">
          <Checkbox
            ...
            onCheckedChange={(v) => set("privacy_accepted")(!!v as any)}
            className="mt-0.5"
          />
